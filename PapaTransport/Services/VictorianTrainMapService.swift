...
final class VictorianTrainMapService: BusDataProviding {
    ...
    func fetchBusInfo(
        ...
        let nearbyRaw = try await VictorianTrainGTFSDatabase.shared.stationsInRegion(
            minLat: minLat,
            maxLat: maxLat,
            minLon: minLon,
            maxLon: maxLon,
    ...
    func fetchTripDetail(
        ...
        let pattern = try await VictorianTrainGTFSDatabase.shared.tripPattern(tripId: departure.tripId)
    ...
    private func fetchTripUpdates() async -> [String: GTFSRTTripUpdate] {
        let apiKey = UserDefaults.standard.string(forKey: VictorianBusService.realtimeAPIKeyDefaultsKey)?
    ...
    private func buildDepartureBoard(
        scheduled: [VictorianTrainGTFSDatabase.ScheduledDeparture],
    ...
    private func realtimeStopDetail(
        for stop: VictorianTrainGTFSDatabase.TripPatternStop,
